---
id: 194
title: Application portal UI — public multi-section form
tier: Opus
depends_on: [183, 193]
feature: P3-002-ai-tenant-screening
---

# 194 — Application portal UI — public multi-section form

## Objective
Create a public, multi-section application form for tenants. Sections: personal info, employment, income, references, and legal consent checkboxes. Mobile-friendly, no auth required. Includes confirmation page with tracking link for status lookup.

## Context
Reference: `features/inprogress/P3-002-ai-tenant-screening/README.md`

Public portal page at `apps/web/app/apply/[propertyId]/page.tsx`. Form submits to task 183 API endpoint. Shows confirmation with tracking ID on success.

## Implementation

### 1. Create application form page

Create `apps/web/app/apply/[propertyId]/page.tsx`:
...
export default function ApplicationPortal({ params }: { params: { propertyId: string } }) {
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select value={formData.employment_status} onValueChange={(value) => handleChange('employment_status', value)}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value={EmploymentStatus.OTHER}>Other</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              <Input
              ...
              <Input
                ...
                onChange={(e) => handleChange('employment_duration_months', e.target.value ? parseInt(e.target.value) : undefined)}
              />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input
                ...
                onChange={(e) => handleChange('annual_income', e.target.value ? parseFloat(e.target.value) : undefined)}
              />
              <Input
                placeholder="Monthly Rent Applying For *"
                type="number"
                value={formData.monthly_rent_applying_for}
                onChange={(e) => handleChange('monthly_rent_applying_for', parseFloat(e.target.value))}
                required
              />
            </div>
          </section>
          ...
          <section>
            ...
            <div className="mb-4 p-4 border border-slate-200 rounded-md">
              ...
              <div className="flex gap-2">
                <Select value={currentReference.relationship} onValueChange={(value) => setCurrentReference({ ...currentReference, relationship: value })}>
                ...
                <Button type="button" onClick={handleAddReference} variant="outline">
                  Add Reference
                </Button>
              </div>
            </div>
          ...
          <section>
            ...
            <label className="flex items-center space-x-2 mb-4">
              <Checkbox
                ...
                onCheckedChange={(checked) => handleChange('has_eviction_history', checked)}
              />
          ...
          <section className="bg-slate-50 p-6 rounded-md border border-slate-200">
            ...
            <div className="space-y-4">
              <label className="flex items-start space-x-3">
                <Checkbox
                  checked={formData.agrees_to_background_check || false}
                ...
                <span className="text-sm">
                  I authorize a background check, including credit and eviction history
                  review. *
                </span>
              </label>
              <label className="flex items-start space-x-3">
                <Checkbox
                  checked={formData.agrees_to_terms || false}
                  onCheckedChange={(checked) =>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-green-100 flex items-center justify-center py-12 px-4">
      <div className="max-w-md bg-white rounded-lg shadow-lg p-8 text-center">
        ...
        <Button
          onClick={() => window.location.href = `/apply/status/${trackingId}`}
...
### 2. Create necessary UI components imports

Ensure the following are available or installed:
- `Button` from `@/components/ui/button`
- `Input` from `@/components/ui/input`
- `Checkbox` from `@/components/ui/checkbox` (task 193)
- `Select` from `@/components/ui/select`

## Acceptance Criteria
1. [ ] Form page at `/apply/[propertyId]` accessible without auth
2. [ ] Multi-section form: personal, employment, income, references, rental history
3. [ ] All required fields marked with asterisk (*)
4. [ ] Checkboxes for:
   - Eviction history toggle (shows/hides details)
   - Background check authorization
   - Terms agreement
5. [ ] References section: add/remove capability
6. [ ] Form validation before submission (client-side)
7. [ ] Form submits to /api/applications (task 183)
8. [ ] Errors displayed in red alert
9. [ ] On success: shows confirmation page with tracking ID
10. [ ] Confirmation page has link to status page
11. [ ] Mobile-friendly (responsive grid layout)
12. [ ] No auth required
13. [ ] Styling consistent with design system (Tailwind CSS)
