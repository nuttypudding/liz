...
4. **Install new components** using the command from `get_add_command_for_items` with `items: ["@shadcn/{name}"]` — run via Bash from the `dashboard/` directory
...
### File Structure
- New UI primitives: `dashboard/src/components/ui/[name].tsx`
- Feature components: `dashboard/src/components/[FeatureName].tsx`
- Shared types: `dashboard/src/types/`
...
- Run the install command via Bash from the `dashboard/` directory
- Check if the component is already installed before adding: read `dashboard/src/components/ui/`
...
- Never modify files outside the `dashboard/` directory unless the plan explicitly says to
