---
id: 182
title: TypeScript types for applications and screening
tier: Haiku
depends_on: []
feature: P3-002-ai-tenant-screening
---

# 182 — TypeScript types for applications and screening

## Objective
Define comprehensive TypeScript interfaces for all screening-related data structures: applications, screening reports, audit logs, and related enums. Enable strict type safety across all screening API routes and UI components.

## Context
Reference: `features/inprogress/P3-002-ai-tenant-screening/README.md`

Create `apps/web/lib/screening/types.ts` with exported types used by all API routes and UI.

## Implementation

### 1. Create types file

Create `apps/web/lib/screening/types.ts`:
...
Create or update `apps/web/lib/screening/index.ts`:
...
## Acceptance Criteria
1. [ ] All enums defined and exported
2. [ ] Application, ScreeningReport, AIScreeningAnalysis interfaces match DB schema
3. [ ] ScreeningFactor interface supports detailed factor breakdown
4. [ ] Public payload types (ApplicationSubmissionPayload, ApplicationDecisionPayload) match form inputs
5. [ ] PublicApplicationStatusResponse hides AI details from applicants
6. [ ] All types use proper TypeScript constraints (literals, enums, optional fields)
7. [ ] Types file imported and used in all API routes (tasks 183–187, 192)
8. [ ] Types file imported and used in all UI components (tasks 194–198)
