---
id: 180
title: Database migration — applications, screening_reports, screening_audit_log tables
tier: Haiku
depends_on: []
feature: P3-002-ai-tenant-screening
---

# 180 — Database migration — applications, screening_reports, screening_audit_log tables

## Objective
Create three core database tables for the tenant screening system: `applications`, `screening_reports`, and `screening_audit_log`. These tables form the foundation for application tracking, AI-powered screening analysis, and compliance audit trails.

## Context
Reference: `features/inprogress/P3-002-ai-tenant-screening/README.md`

Supabase PostgreSQL schema. Migrations run locally with `supabase db pull` and `supabase db push`, deployed to production via Vercel.
...
### 2. Create screening_reports table
...
### 3. Create screening_audit_log table
...
create policy "Applicants can view their own applications"
  ...
  using (auth.uid()::text = tracking_id or false); -- tracking_id not a user id, but this is placeholder
...
2. [ ] All constraints (FK, unique, check) in place
3. [ ] Indexes created for common query patterns (property_id, landlord_id, status, created_at, tracking_id)
4. [ ] RLS policies enabled and tested (landlord access, public create, audit trail)
5. [ ] Migration file saved to `supabase/migrations/`
6. [ ] Local database updated with `supabase db push`
7. [ ] Schema synchronized with TypeScript types (task 182)
